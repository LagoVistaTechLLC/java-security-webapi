...
public class UserRowFactoryImp<R extends UserRow> implements UserRowFactory<R> {
	protected UserRowFactoryImp() {}
	
	@SuppressWarnings("unchecked")
	public R create() { return (R) new UserRowImp(new HashMap<String, Object>()); }
	...
	public R loadByUserName(Connection conn, java.lang.String value) throws Exception {
		String sql = 
			"SELECT * " + 
			"FROM " + conn.getAdapter().quoteIdentifier("Users") + " " + 
			"WHERE " + conn.getAdapter().quoteIdentifier("User Name") + "=@Value";
		...
		Table<R> table = conn.fill(this, sql, params);
		if(table.size() != 1)
			throw new RecordNotFoundException("Could not load unique row for 'User Name' having a value of " + value.toString() + "!");
